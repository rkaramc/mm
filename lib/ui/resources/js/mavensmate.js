...
function renderTree(treeOffset, packageName) {
	try {
                ...
                if (json[rec.parent.data.text] === undefined) {
                    try {
                        if (Object.prototype.toString.call(rec.data.type.childXmlNames) === '[object Array]') {
                            if (rec.data.type.childXmlNames.length == 0 && !rec.data.type.inFolder) {
                                json[rec.data.text] = '*'
                            } else {
                                json[rec.data.text] = []
                            }
                        } else {
                            if (rec.data.type.inFolder) {
                            	json[rec.data.text] = []
                            } else {
                            	json[rec.data.text] = '*'
                            }
...
function show_message(message, mtype, showCloseButton) {
	...
	if (showCloseButton) {
		message += '<br/><a href="#" class="btn btn-info btn-embossed btn-wide" onclick="window.close()">Done</a>'
	}
	$("#error_message").parent().attr('class', 'alert')
	$("#error_message").parent().addClass('alert-'+mtype)
	$("#error_message").html(message)
