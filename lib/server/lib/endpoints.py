...
import json
sys.path.append('../')
import lib.server.lib.util as util
from lib.server.lib.util import BackgroundWorker
import lib.server.lib.config as gc
...
def get_request_params(request_handler):
    ...
    if request_handler.command == 'POST':
        ...
        params = urlparse.parse_qs(urlparse.urlparse(request_handler.path).query)
        for key in params:
            if '[]' in key:
